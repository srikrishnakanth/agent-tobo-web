// motion.js emitter: capability-aware runtime for micro-interactions, scroll reveals, parallax,
// carousel keyboard support, theme switching and automatic 3D/heavy-animation degradation.
// Emitted as a classic script (HTML) or an ES module (React/Next) from the same body.
export function emitMotionJs(tokens, selection, { module = false } = {}) {
  const cfg = {
    level: selection.motion, scroll: !!tokens.motion.scroll, parallax: !!tokens.motion.parallax, threeD: tokens.motion.threeD || false, carousel: !!tokens.motion.carousel,
    fallback: tokens.motion.fallback, lowEndCores: 4, lowEndMemoryGB: 4, perfBudgetLongFrameRatio: 0.3,
  };
  const body = `
const KK_CONFIG = ${JSON.stringify(cfg)};
...
`;
  if (module) {
    return `// KK-UI-GOVERNOR motion runtime (ES module). Import and call initKkMotion() once on the client.\n${body}\nexport { kkInit as initKkMotion, kkTheme, kkCapabilities, kkDecideMotion, kkDecide3D, KK_CONFIG };\nexport default kkInit;\n`;
  }
  return `/* KK-UI-GOVERNOR motion runtime — generated ${tokens.meta.generatedAt}. Reduced-motion, save-data, low-end and forced-colors aware. */\n(function () {\n'use strict';\n${body}\nwindow.kkTheme = kkTheme;\nwindow.kkGovernor = { init: kkInit, capabilities: kkCapabilities, config: KK_CONFIG };\nkkInit();\n})();\n`;
}

export function emitAttribution(tokens, catalog) {
  const lines = ['# Design attribution', '', 'Generated by KK-UI-GOVERNOR. Design intent for this theme references the following upstream sources; nothing here is copied verbatim unless listed as a file, and every referenced license is preserved in the Design Vault (`DESIGN-VAULT/sources/<id>/LICENSE`).', ''];
  lines.push('| Asset | Category | Upstream | Commit | License | Use |', '|---|---|---|---|---|---|');
  for (const a of tokens.assets) lines.push(`| ${a.id} | ${a.category} | ${a.upstream?.repo || ''} ${a.upstream?.path || ''} | ${(a.upstream?.commit || '').slice(0, 12)} | ${a.license} | ${a.note || ''} |`);
  lines.push('', `Primary colour: ${tokens.color.primary} (${tokens.color.primarySource}).`, '');
  return lines.join('\n') + '\n';
}
